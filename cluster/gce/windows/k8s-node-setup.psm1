...
$INFRA_CONTAINER = 'mcr.microsoft.com/k8s/core/pause:1.2.0'
...
function Pull-InfraContainer {
  ...
  $PULL = "pull"
  $INSPECT = "inspect"
  if (${env:CONTAINER_RUNTIME} -ne "docker") {
    $COMMAND = "crictl"
    $INSPECT = "inspecti"
  }
  $name, $label = $INFRA_CONTAINER -split ':',2
  if (-not ("$(& $COMMAND $LIST)" -match "$name.*$label")) {
    & $COMMAND $PULL $INFRA_CONTAINER
    if (!$?) {
      throw "Error running '$COMMAND $PULL $INFRA_CONTAINER'"
    }
  }
  $inspect = "$(& $COMMAND $INSPECT $INFRA_CONTAINER | Out-String)"
...
#   The HNS network for pod networking has been configured
#     (Configure-HostNetworkingService).
#   Containerd is installed (Install-Containerd).
#
# Required ${kube_env} keys:
#   DNS_SERVER_IP
#   DNS_DOMAIN
#   CLUSTER_IP_RANGE
#   SERVICE_CLUSTER_IP_RANGE
function Configure-Containerd-CniNetworking {
  $l2bridge_conf = "${env:CNI_CONFIG_DIR}\l2bridge.conf"
  if (-not (ShouldWrite-File ${l2bridge_conf})) {
    return
  }
